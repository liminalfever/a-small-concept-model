...
class SmallConceptModel(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        returns: Tensor of shape (B, T, d_embed), prediction for target sequence of embeddings
        """
        x = self.in_proj(x)
        B, T, D = x.shape
        ...
        assert (
            T <= self.max_seq_len
        ), f"Sequence length {T} > max_seq_len {self.max_seq_len}"

        x = x + self.pos_emb[:T].unsqueeze(
            0
        )  # (T, D) -> (1, T, D) broadcast to (B, T, D)
        x = x.permute(1, 0, 2)  # Transformer expects (T, B, D)

        tgt_mask = nn.Transformer.generate_square_subsequent_mask(T).to(x.device)

        memory = torch.zeros(
            1, B, D, device=x.device
        )  # memory: omitted or zeros, but nn.TransformerDecoder requires memory: set to zeros.
